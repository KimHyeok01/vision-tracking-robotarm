...
# 프로젝트 마스터 가이드: 비전 추종 4축 로봇암
본 가이드는 상위 제어(PC/RPi)의 비전 인식과 하위 제어(STM32)의 정밀 모터 제어를 결합한 분산 제어 시스템 구축을 목표로 합니다.
...
## 💡 포트폴리오 활용 전략 (Career Point)
이 프로젝트를 통해 증명할 수 있는 역량입니다.
 * **시스템 통합:** 로봇융합전공 지식을 바탕으로 기구-회로-소프트웨어 전 과정을 조율함.
 * **유지보수 및 안정성:** 네이버랩스에서의 로봇 유지보수 경험을 살려, 장애 발생 시 복구가 용이한 구조적 설계 강조.
 * **전문성:** 단순 PWM 제어를 넘어 시리얼 버스 통신 프로토콜과 ROS 2 미들웨어를 실무 수준으로 다룸.
**주의 사항:** 3D 프린팅 시 출력물의 결을 고려하여 하중을 가장 많이 받는 2축(Shoulder) 브래킷의 파손을 방지하십시오.
